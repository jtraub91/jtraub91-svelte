<header class="w-full flex flex-row">
  <div class="top-0 left-0 h-20 w-20">
    <a href="/">
      <img src="/images/yoshi-github-jtraub91.jpg"/>
    </a>
  </div>
  <a href="/blog" class="mx-4 mt-auto mb-0 font-mono text-blue-500 hover:text-green-400 text-4xl">
    blog
  </a>
  <a href="/links" class="mx-4 mt-auto mb-0 font-mono text-blue-500 hover:text-green-400 text-lg flex">
    <span class="mb-0 mt-auto mx-auto">
      links
    </span>
  </a>
  <a href="mailto:jtraub91@gmail.com" class="ml-auto mr-4 mt-auto mb-0 font-mono text-blue-500 hover:text-green-400 text-4xl">
    contact
  </a>
</header>